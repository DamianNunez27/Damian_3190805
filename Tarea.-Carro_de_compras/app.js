...
function mostrarMenu() {
    ...
    menu += (productos.length + 3) + ". Agregar Nuevo Producto\n"; // Opción para agregar un nuevo producto
    return menu;
}
...
function agregarProducto() {
    ...
    var precio = parseFloat(prompt("Ingrese el precio del nuevo producto:"));

    if (nombre && !isNaN(precio) && precio > 0) {
        productos.push({ nombre: nombre, precio: precio });
        console.log('Producto "' + nombre + '" agregado a la lista de productos.');
    } else {
        console.log('Entrada inválida. Asegúrese de ingresar un nombre válido y un precio positivo.');
    }
}
...
} while (true); // El bucle continúa hasta que elige "Salir"
